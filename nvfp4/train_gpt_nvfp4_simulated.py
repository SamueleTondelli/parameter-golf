...
def attention_pytorch(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
...
class Hyperparameters:
    ...
    trigram_enabled = bool(
        int(os.environ.get("TRIGRAM", "0"))
    )  # TrigramHash (off by default, risky)
    xsa_last_n = int(
        os.environ.get("XSA_LAST_N", 11)
    )  # XSA on ALL layers (our novel contribution)
    ...
    value_residual = bool(
        int(os.environ.get("VALUE_RESIDUAL", "0"))
    )  # VRL with sigmoid gates (off by default, risky)
...
# NVFP4 E2M1 representable values: 0, 0.5, 1, 1.5, 2, 3, 4, 6 (and negatives)
...
def _quantize_nvfp4_block(block: Tensor, scale: float) -> Tensor:
    """Quantize a block to nearest NVFP4 representable value."""
    scaled = block / scale
    # Clamp to NVFP4 range
    clamped = scaled.clamp(-NVFP4_MAX, NVFP4_MAX)
    # Snap to nearest representable value
    diffs = (clamped.unsqueeze(-1) - _NVFP4_VALUES.to(clamped.device)).abs()
    indices = diffs.argmin(dim=-1)
    return _NVFP4_VALUES.to(clamped.device)[indices]


def quantize_nvfp4_per_row_2d(t: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    """NVFP4 quantization with 1D blocks of 16, per-row FP8 block scales + per-tensor FP32 scale.
    Returns: (quantized_values_fp32, block_scales_fp32, tensor_scale_fp32)
    The quantized values are stored as float32 containing only NVFP4-representable values.
    """
    t32 = t.float()
    rows, cols = t32.shape
    # Pad columns to multiple of NVFP4_BLOCK_SIZE
    pad_cols = (NVFP4_BLOCK_SIZE - cols % NVFP4_BLOCK_SIZE) % NVFP4_BLOCK_SIZE
    if pad_cols > 0:
        t32 = F.pad(t32, (0, pad_cols))
    padded_cols = t32.shape[1]
    # Reshape into blocks: [rows, num_blocks, block_size]
    num_blocks = padded_cols // NVFP4_BLOCK_SIZE
    blocks = t32.reshape(rows, num_blocks, NVFP4_BLOCK_SIZE)
    # Per-block amax
    block_amax = blocks.abs().amax(dim=-1)  # [rows, num_blocks]
    # Per-tensor scale: ensures block scales fit in FP8 E4M3 range
    global_amax = block_amax.max().clamp_min(1e-12)
    tensor_scale = global_amax / (NVFP4_MAX * FP8_E4M3_MAX)
    tensor_scale = tensor_scale.clamp_min(1e-12)
    # Per-block scales (what would be stored as FP8 E4M3)
    block_scales = (block_amax / (NVFP4_MAX * tensor_scale)).clamp_min(
        1e-12
    )  # [rows, num_blocks]
    # Clamp block scales to FP8 E4M3 range
    block_scales = block_scales.clamp(max=FP8_E4M3_MAX)
    # Quantize each block
    effective_scales = (tensor_scale * block_scales).unsqueeze(
        -1
    )  # [rows, num_blocks, 1]
    q = blocks / effective_scales
    q = q.clamp(-NVFP4_MAX, NVFP4_MAX)
    # Snap to nearest representable NVFP4 value
    nvfp4_vals = _NVFP4_VALUES.to(q.device)
    diffs = (q.unsqueeze(-1) - nvfp4_vals).abs()
    indices = diffs.argmin(dim=-1)
    q_snapped = nvfp4_vals[indices]
    # Pack back: store quantized values and scales
    q_flat = q_snapped.reshape(rows, padded_cols)
    if pad_cols > 0:
        q_flat = q_flat[:, :cols]
    # Store as int8 with a lookup: map NVFP4 values to 4-bit indices (0-15)
    # For storage, pack two 4-bit values per int8 byte
    return q_snapped.reshape(rows, padded_cols), block_scales, tensor_scale, cols


def dequantize_nvfp4_per_row_2d(
    q_snapped: Tensor, block_scales: Tensor, tensor_scale: Tensor, orig_cols: int
) -> Tensor:
    """Dequantize NVFP4 back to float32."""
    rows = q_snapped.shape[0]
    padded_cols = q_snapped.shape[1]
    num_blocks = padded_cols // NVFP4_BLOCK_SIZE
    blocks = q_snapped.reshape(rows, num_blocks, NVFP4_BLOCK_SIZE)
    effective_scales = (tensor_scale * block_scales).unsqueeze(-1)
    deq = blocks * effective_scales
    deq = deq.reshape(rows, padded_cols)
    return deq[:, :orig_cols]


def _nvfp4_encode_tensor(q_snapped: Tensor) -> Tensor:
    """Pack NVFP4 quantized values (as floats) into int8 using a 4-bit index encoding.
    Two values per byte. Returns int8 tensor with half the elements."""
    # Map float values to 4-bit indices
    nvfp4_vals = _NVFP4_VALUES.to(q_snapped.device)
    flat = q_snapped.reshape(-1)
    diffs = (flat.unsqueeze(-1) - nvfp4_vals).abs()
    indices = diffs.argmin(dim=-1).to(torch.uint8)  # 0-15
    # Pack pairs into bytes: high nibble + low nibble
    if indices.numel() % 2 != 0:
        indices = F.pad(indices, (0, 1))  # pad to even
    high = indices[0::2] << 4
    low = indices[1::2]
...
def _nvfp4_decode_tensor(
    packed: Tensor, num_elements: int, device: torch.device
) -> Tensor:
    """Unpack int8 packed NVFP4 values back to float tensor."""
    nvfp4_vals = _NVFP4_VALUES.to(device)
    raw = packed.to(torch.uint8)
    high = (raw >> 4) & 0x0F
    low = raw & 0x0F
    indices = torch.stack([high, low], dim=-1).reshape(-1)[:num_elements]
    return nvfp4_vals[indices.long()]


def quantize_state_dict_nvfp4(state_dict: dict[str, Tensor]) -> tuple[dict, dict]:
    """Quantize large 2D float tensors to NVFP4, keep small/control tensors as passthrough."""
    ...
    for name, tensor in state_dict.items():
        ...
        if not t.is_floating_point():
            ...
            continue
        # Small tensors or control tensors: keep in fp16
        if t.numel() <= 65536 or any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            result[name] = (
                t.to(torch.float16)
            ...
            stats["passthrough_tensors"] += 1
            stats["nvfp4_payload_bytes"] += int(result[name].numel()) * int(
                result[name].element_size()
            )
            continue
        # Large 2D float tensors: NVFP4 quantize
        if t.ndim == 2:
            q_snapped, block_scales, tensor_scale, orig_cols = (
                quantize_nvfp4_per_row_2d(t)
            )
            packed = _nvfp4_encode_tensor(q_snapped)
            result[name + ".nvfp4_packed"] = packed
            result[name + ".block_scales"] = block_scales.to(torch.float16)
            result[name + ".tensor_scale"] = tensor_scale
            meta[name] = {
                "type": "nvfp4",
                "orig_shape": list(t.shape),
                "orig_dtype": str(t.dtype).removeprefix("torch."),
                "padded_cols": q_snapped.shape[1],
                "num_elements": int(q_snapped.numel()),
            }
            stats["nvfp4_tensors"] += 1
            payload = int(packed.numel()) + int(block_scales.numel()) * 2 + 4
            stats["nvfp4_payload_bytes"] += payload
        else:
            # 1D or >2D float: keep as fp16
            result[name] = t.to(torch.float16)
            meta[name] = "passthrough_fp16"
            stats["passthrough_tensors"] += 1
            stats["nvfp4_payload_bytes"] += int(result[name].numel()) * 2
    return {"__quant_format__": "nvfp4_v1", "data": result, "meta": meta}, stats


def dequantize_state_dict_nvfp4(obj: dict) -> dict[str, Tensor]:
    """Dequantize NVFP4 state dict back to float tensors."""
    result = obj["data"]
    meta = obj["meta"]
    out: dict[str, Tensor] = {}
    for name, info in meta.items():
        if info in ("passthrough", "passthrough_fp16"):
            out[name] = result[name]
            continue
        if isinstance(info, dict) and info["type"] == "nvfp4":
            packed = result[name + ".nvfp4_packed"]
            block_scales = result[name + ".block_scales"].float()
            tensor_scale = result[name + ".tensor_scale"].float()
            orig_shape = info["orig_shape"]
            padded_cols = info["padded_cols"]
            num_elements = info["num_elements"]
            orig_dtype = getattr(torch, info["orig_dtype"])
            # Decode packed values
            q_snapped = _nvfp4_decode_tensor(packed, num_elements, block_scales.device)
            q_snapped = q_snapped.reshape(orig_shape[0], padded_cols)
            # Dequantize
            deq = dequantize_nvfp4_per_row_2d(
                q_snapped, block_scales, tensor_scale, orig_shape[1]
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                rows, cols = w32.shape
                pad_cols = (
                    NVFP4_BLOCK_SIZE - cols % NVFP4_BLOCK_SIZE
                ) % NVFP4_BLOCK_SIZE
                w_padded = F.pad(w32, (0, pad_cols)) if pad_cols > 0 else w32
                num_blocks = w_padded.shape[1] // NVFP4_BLOCK_SIZE
                blocks = w_padded.reshape(rows, num_blocks, NVFP4_BLOCK_SIZE)
                block_amax = blocks.abs().amax(dim=-1)
                global_amax = block_amax.max().clamp_min(1e-12)
                tensor_scale = global_amax / (NVFP4_MAX * FP8_E4M3_MAX)
                tensor_scale = tensor_scale.clamp_min(1e-12)
                block_scales = (
                    (block_amax / (NVFP4_MAX * tensor_scale))
                    .clamp_min(1e-12)
                ...
                indices = diffs.argmin(dim=-1)
                q_snapped = nvfp4_vals[indices]
                w_q = (q_snapped * effective_scales).reshape(rows, w_padded.shape[1])
                if pad_cols > 0:
                    w_q = w_q[:, :cols]
                w_q = w_q.to(x.dtype)
            w = w + (w_q - w).detach()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if value_residual:
            self.vrl_alpha = nn.Parameter(
                torch.zeros(1, dtype=torch.float32)
            )  # sigmoid gate (PR #569 style)
    ...
    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        ...
        vn = F.normalize(v, dim=-1).unsqueeze(
            -2
        )  # [B, T, Hkv, 1, D] -- broadcast ready
...
class BigramHashEmbedding(nn.Module):
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out[..., :2] = mod
        out[..., 2:] = (
            36313 * t[..., 2:] ^ 27191 * t[..., 1:-1] ^ 51497 * t[..., :-2]
        ) % mod
        return out.long()
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self._ve_target_dim = num_kv_heads * (
            model_dim // num_heads
        )  # kv_dim for value projection
...
def _rebank_state_dict(
    sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]
) -> dict[str, Tensor]:
    ...
    out["mlp_up_bank"] = torch.stack(up_slices).to(
        dtype=template_sd["mlp_up_bank"].dtype
    )
    out["mlp_down_bank"] = torch.stack(down_slices).to(
...
def main() -> None:
    ...
    # Simulated native NVFP4: always enable NVFP4 fake-quant from step 0
    CastedLinear._qat_enabled = True
    log0("nvfp4_simulated: NVFP4 QAT enabled from step 0 (software emulation)")
    base_model = (
        GPT(
    ...
    torch.cuda.synchronize()
    final_val_loss = diag_val_loss
    final_val_bpb = diag_val_bpb
    log0(
    ...
    # NVFP4 quantization (no Hessian/GPTQ needed)
    log0("nvfp4: quantizing weights to NVFP4 E2M1 format...")
    quant_obj, quant_stats = quantize_state_dict_nvfp4(unbanked_sd)
    log0(
        f"nvfp4: {quant_stats['nvfp4_tensors']} tensors quantized, "
        f"{quant_stats['passthrough_tensors']} passthrough, "
        f"baseline={quant_stats['baseline_bytes'] / (1024 * 1024):.2f}MB "
        f"nvfp4_payload={quant_stats['nvfp4_payload_bytes'] / (1024 * 1024):.2f}MB"
    )
    ...
    if master_process:
        with open("final_model.nvfp4.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model nvfp4+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size nvfp4+lzma: {quant_file_bytes + code_bytes} bytes")
        final_artifact_size = quant_file_bytes + code_bytes
    if distributed:
        dist.barrier()
    with open("final_model.nvfp4.ptz", "rb") as f:
        quant_blob_disk = f.read()
    quant_state = torch.load(
        io.BytesIO(lzma.decompress(quant_blob_disk)),
        map_location="cpu",
    )
    deq_unbanked = dequantize_state_dict_nvfp4(quant_state)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_nvfp4_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(
        f"final_nvfp4_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}"
    )
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_nvfp4_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(
            f"final_nvfp4_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}"
        )
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_nvfp4_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(
            f"final_nvfp4_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}"
        )
    if distributed:
    ...
    # Final summary
    log0("=" * 60)
    log0("FINAL RESULTS SUMMARY")
    log0("=" * 60)
    log0(f"Experiment: Simulated Native NVFP4 (QAT from step 0, then quantize)")
    log0(
    ...
    log0(
        ...
        else "Final Val Loss: N/A"
    )
    log0(
        f"Final Val BPB: {final_val_bpb:.6f}"
        if final_val_bpb is not None
        else "Final Val BPB: N/A"
    )
    log0(
        f"Final Artifact Size: {final_artifact_size} bytes ({final_artifact_size / 1024 / 1024:.2f} MB)"
        if final_artifact_size is not None
        else "Final Artifact Size: N/A"
    )
    log0("=" * 60)
