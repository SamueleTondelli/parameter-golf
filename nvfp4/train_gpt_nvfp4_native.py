...
class Hyperparameters:
    ...
    nvfp4_keep_last_n_hp = int(os.environ.get("NVFP4_KEEP_LAST_N_HP", 2))
...
class TELinear(nn.Module):
    """Wrapper around Transformer Engine Linear for NVFP4 native training."""

    def __init__(self, in_features: int, out_features: int, bias: bool = False):
        super().__init__()
        self.te_linear = te.Linear(in_features, out_features, bias=bias)
        self.te_linear.bfloat16()
...
_use_te_linear: bool = bool(int(os.environ.get("USE_TE_LINEAR", "1")))
...
# --- NVFP4 native model (per-layer TELinear instead of weight banks) ---


class NVFP4Block(nn.Module):
    """Transformer block with TELinear layers for native NVFP4 training."""
    ...
    def __init__(
        ...
        LinearCls = TELinear if use_te else CastedLinear
        ...
        self.fc = LinearCls(dim, mlp_dim, bias=False)
        self.mlp_proj = LinearCls(mlp_dim, dim, bias=False)
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
    ...
    def forward(self, x, x0, v_embed=None, v0=None):
        ...
        if self.use_xsa:
            y = self._xsa_efficient(y, v)
        if self.gated_attention:
            gate = torch.sigmoid(self.attn_gate(x_in)).unsqueeze(-1)
            y = y * gate
        attn_out = self.proj(y.reshape(bsz, seqlen, dim))
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        # MLP
        h = self.mlp_norm(x_out) * self.ln_scale_factor
        mlp_out = self.mlp_proj(F.leaky_relu(self.fc(h), negative_slope=0.5).square())
...
class GPT_NVFP4(nn.Module):
    """GPT model using per-layer TELinear for native NVFP4 training.
    Last `keep_last_n_hp` layers use CastedLinear (higher precision) instead of TELinear."""
    ...
    def __init__(
        ...
        # Build blocks: last `keep_last_n_hp` use CastedLinear
        self.blocks = nn.ModuleList()
        for i in range(num_layers):
            use_te = i < num_layers - keep_last_n_hp
            block = NVFP4Block(
                model_dim,
                num_heads,
                num_kv_heads,
                ...
                qk_gain_init,
                layer_idx=i,
                xsa_last_n=(xsa_last_n if i >= num_layers - xsa_last_n else 0),
                rope_dims=rope_dims,
                ln_scale=ln_scale,
                dtg=dtg,
                gated_attention=gated_attention,
    ...
    def _init_weights(self) -> None:
        ...
        for i, block in enumerate(self.blocks):
            # Q, K, V: orthogonal init (cast to fp32 for QR, then back)
            for layer in [block.c_q, block.c_k, block.c_v]:
                w = layer.weight
                if w.shape[0] >= 64 and w.shape[1] >= 64:
                    orig_dtype = w.dtype
                    w.data = w.data.float()
                    nn.init.orthogonal_(w, gain=1.0)
                    w.data = w.data.to(orig_dtype)
            # fc (MLP up): orthogonal
            w = block.fc.weight
            if w.shape[0] >= 64 and w.shape[1] >= 64:
                orig_dtype = w.dtype
                w.data = w.data.float()
                nn.init.orthogonal_(w, gain=1.0)
                w.data = w.data.to(orig_dtype)
            # proj (attn out): zero init, scaled
            nn.init.zeros_(block.proj.weight)
            block.proj.weight.data.mul_(proj_scale)
            # mlp_proj (MLP down): zero init, scaled
            nn.init.zeros_(block.mlp_proj.weight)
            block.mlp_proj.weight.data.mul_(proj_scale)
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear) and not isinstance(
                module, (CastedLinear, TELinear)
            ):
                if getattr(module, "_zero_init", False):
                    nn.init.zeros_(module.weight)
                elif (
...
def main() -> None:
    ...
    if _use_te_linear:
        ...
        base_model = (
            GPT_NVFP4(
                ...
                keep_last_n_hp=args.nvfp4_keep_last_n_hp,
            )
            .to(device)
            .bfloat16()
        )
        ...
        log0(f"nvfp4:using GPT_NVFP4 model, keep_last_n_hp={args.nvfp4_keep_last_n_hp}")
    else:
        base_model = (
            GPT(
                vocab_size=args.vocab_size,
    ...
    if _use_te_linear:
        # TE's Linear.forward uses torch.compiler.disable, incompatible with fullgraph=True
    ...
    # Optimizer split
    if _use_te_linear:
        # NVFP4 path: per-layer weight matrices -> Muon, everything else -> Adam
        matrix_params = []
        for name, p in base_model.named_parameters():
            if p.ndim == 2 and p.shape[0] >= 64 and p.shape[1] >= 64:
                if any(
                    sub in name
                    for sub in ("c_q.", "c_k.", "c_v.", "proj.", "fc.", "mlp_proj.")
                ):
    ...
    xsa_layers = (
        [i for i, b in enumerate(base_model.blocks) if b.use_xsa]
        if _use_te_linear
        else [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    )
    ...
    # Int8 quantized save for comparison
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    int8_obj, int8_stats = quantize_state_dict_int8(sd_cpu)
    if master_process:
        int8_buf = io.BytesIO()
        torch.save(int8_obj, int8_buf)
        int8_raw = int8_buf.getvalue()
        if _COMPRESSOR == "zstd":
            int8_blob = zstandard.ZstdCompressor(level=19).compress(int8_raw)
            ext = "ptz"
        else:
            int8_blob = zlib.compress(int8_raw, 9)
            ext = "ptz"
        with open(f"final_model.int8.{ext}", "wb") as f:
            f.write(int8_blob)
        log0(f"Serialized model int8+{_COMPRESSOR}: {len(int8_blob)} bytes")
        log0(f"int8_stats: {int8_stats}")
        final_artifact_size = len(int8_blob) + code_bytes
        log0(f"Total artifact size: {final_artifact_size} bytes")
    # Int8 roundtrip eval
    if distributed:
        dist.barrier()
    deq_sd = dequantize_state_dict_int8(int8_obj)
    base_model.load_state_dict(deq_sd, strict=True)
    compiled_eval = base_model if _use_te_linear else torch.compile(base_model, dynamic=False, fullgraph=True)
    torch.cuda.synchronize()
    ...
    log0(
        f"final_int8_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}"
    )
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        log0(
            f"final_int8_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}"
        )
    if distributed:
    ...
    log0("=" * 60)
    log0(f"Experiment: Native NVFP4")
    log0(
