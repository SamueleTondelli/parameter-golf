...
class Hyperparameters:
    ...
    use_pete = bool(int(os.environ.get("USE_PETE", "0")))
...
class PolynomialBlock(nn.Module):
    """Deterministic Fourier features from token IDs - zero parameters."""
...
class PETEEmbedding(nn.Module):
    """PETE-style parameter-efficient embedding (polynomial + MLP projector)."""

    def __init__(self, vocab_size: int, model_dim: int, base: float = 10000.0):
        super().__init__()
        self.poly = PolynomialBlock(
            model_dim, vocab_size, base=base, index_mode="normalized"
        )
        hidden = model_dim // 4
        self.proj = CastedLinear(model_dim, hidden * 2)
        self.proj_out = CastedLinear(hidden, model_dim)
        self.norm = RMSNorm()

    def forward(self, input_ids: Tensor) -> Tensor:
        x_poly = self.poly(input_ids)
        x = self.proj(x_poly)
        x, gate = x.chunk(2, dim=-1)
        x = x * F.gelu(gate)
        x = self.proj_out(x)
        return self.norm(x + x_poly)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.use_pete = use_pete
        if use_pete:
            self.tok_emb = PETEEmbedding(vocab_size, model_dim)
        else:
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings and not self.use_pete:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            if self.use_pete:
                logits_proj = F.linear(x, self.tok_emb.proj_out.weight)
            else:
...
def main() -> None:
    ...
    if args.use_pete:
        pete_params = list(base_model.tok_emb.parameters())
        pete_matrix = [p for p in pete_params if p.ndim == 2]
        pete_scalar = [p for p in pete_params if p.ndim < 2]
        matrix_params.extend(pete_matrix)
        scalar_params.extend(pete_scalar)
        optimizer_tok = None
    else:
    ...
    if optimizer_tok is not None:
        optimizers.insert(0, optimizer_tok)
    ...
    embed_lr_str = (
        "N/A (PETE)"
        if args.use_pete
        else str(args.tied_embed_lr if args.tie_embeddings else args.embed_lr)
    )
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{embed_lr_str} "
