...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        mlp_mults: list[float] | None = None,
    ):
        super().__init__()
        ...
        # Variable MLP dimensions per layer
        if mlp_mults is None:
            mlp_mults = [mlp_mult] * num_layers
        if len(mlp_mults) != num_layers:
            raise ValueError(
                f"mlp_mults length {len(mlp_mults)} must match num_layers {num_layers}"
            )
        self.mlp_dims = [int(m * model_dim) for m in mlp_mults]
        self.mlp_up_bank = nn.ParameterList(
            [nn.Parameter(torch.empty(d, model_dim)) for d in self.mlp_dims]
        )
        self.mlp_down_bank = nn.ParameterList(
            [nn.Parameter(torch.empty(model_dim, d)) for d in self.mlp_dims]
        )
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(n):
            ...
            nn.init.orthogonal_(self.mlp_up_bank[i].data, gain=1.0)  # MLP up
            nn.init.zeros_(self.mlp_down_bank[i].data)  # MLP down (zero init)
            # Scale proj layers (out_proj and mlp_down are "proj" layers)
            self.qo_bank.data[n + i].mul_(proj_scale)
            self.mlp_down_bank[i].data.mul_(proj_scale)
...
def collect_hessians_from_tokens(hessian_model, token_seqs, device):
    ...
    for name, module in hessian_model.named_modules():
        if isinstance(module, CastedLinear):
            ...
            hessians[param_name] = torch.zeros(
                cols, cols, dtype=torch.float32, device="cpu"
            )
    ...
    for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
        ...
        for i1 in range(0, cols, block_size):
            ...
            for i in range(count):
                w = W1[:, i]
                d = Hinv1[i, i]
                q = torch.clamp(torch.round(w / sf), -clip_range, clip_range).to(
                    torch.int8
                )
                Q1[:, i] = q
                err = (w - q.float() * sf) / d
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    ...
    for name, tensor in sd.items():
        ...
        elif name.startswith("mlp_up_bank."):
            idx = int(name.split(".")[1])
            out[f"blocks.{idx}.mlp.fc.weight"] = tensor
        elif name.startswith("mlp_down_bank."):
            idx = int(name.split(".")[1])
            out[f"blocks.{idx}.mlp.proj.weight"] = tensor
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(
    ...
    for i in range(n):
        ...
        if fk in sd:
            out[f"mlp_up_bank.{i}"] = sd[fk]
            consumed.add(fk)
        dk = f"blocks.{i}.mlp.proj.weight"
        if dk in sd:
            out[f"mlp_down_bank.{i}"] = sd[dk]
            consumed.add(dk)
...
class _HessianGPT(nn.Module):
    ...
    def _get_ve(self, layer_idx, input_ids, ve_cache):
        ...
        if "ve" not in ve_cache:
            ve_cache["ve"] = self.ve_shared(input_ids)
        ve_idx = self.ve_layer_indices.index(layer_idx)
        return ve_cache["ve"] * self.ve_layer_scales[ve_idx].to(
            dtype=ve_cache["ve"].dtype
        )
...
def collect_hessians(
    hessian_model, train_loader, args, device, grad_accum_steps, num_batches=256
):
...
def main() -> None:
    ...
    for p in base_model.mlp_up_bank:
        p.data = p.data.float()
    for p in base_model.mlp_down_bank:
        p.data = p.data.float()
    ...
    matrix_params = (
        ...
        + list(base_model.mlp_up_bank)
        + list(base_model.mlp_down_bank)
    )
    ...
    log0(f"mlp_dims:{base_model.mlp_dims}")
    log0(
    ...
    hessian_model.load_state_dict(
        {
            ...
            if k in hessian_model.state_dict()
        },
        strict=False,
    )
    ...
    log0(
        f"gptq:generated {len(ar_tokens)} sequences in {time.perf_counter() - t_gen:.1f}s"
    )
    ...
    if ones_info:
        ...
        log0(
            f"selective_prune: {len(ones_info)} ±1 candidates, unpruned={no_sz / (1024 * 1024):.2f}MB target={target_mb}MB"
        )
            ...
            if full_sz > target_bytes:
                ...
                while lo < hi:
                    ...
                    if sz <= target_bytes:
                        ...
                        lo = mid + 1
                log0(
                    f"selective_prune: pruning {lo}/{len(ones_info)} ±1 values ({100 * lo / len(ones_info):.1f}%) to fit {target_mb}MB"
                )
    ...
    for p in eval_model.mlp_up_bank:
        p.data = p.data.float()
    for p in eval_model.mlp_down_bank:
        p.data = p.data.float()
