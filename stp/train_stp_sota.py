...
class Hyperparameters:
    ...
    # STP regularizer. Set to 0.0 to disable.
    stp_lambda = float(os.environ.get("STP_LAMBDA", 0.02))
...
def main() -> None:
    ...
    use_stp = args.stp_lambda > 0.0
    log0(f"stp_lambda:{args.stp_lambda} stp_enabled:{use_stp}")
    ...
    while True:
        ...
        if should_log_train:
            stp_part = f" stp_loss:{train_stp_loss.item():.4f}" if use_stp else ""
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f}{stp_part} "
    ...
    if master_process:
        ...
        with open(results_path, "a", newline="") as f:
            ...
            writer.writerow([args.run_id, args.stp_lambda, f"{train_ntp_loss.item():.6f}", f"{final_val_bpb:.8f}"])
