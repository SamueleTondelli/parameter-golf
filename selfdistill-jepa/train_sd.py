    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                embeddings = encoder(x).detach()
                logits = lm_head(embeddings).detach()
...
class LMHead(nn.Module):
    def __init__(self, model_dim: int, vocab_size: int, logit_softcap: float):
        ...
        self.proj = CastedLinear(model_dim, vocab_size, bias=False)
        self.proj._zero_init = True
        nn.init.zeros_(self.proj.weight)

    def forward(self, x: Tensor) -> Tensor:
        x = x.reshape(-1, x.size(-1))
        logits_proj = self.proj(x)
...
def main() -> None:
    ...
    compiled_encoder = torch.compile(base_encoder, dynamic=False, fullgraph=True)
    encoder: nn.Module = (
        DDP(compiled_encoder, device_ids=[local_rank], broadcast_buffers=False)
        if distributed
        else compiled_encoder
    )

    # Encoder optimizer split:
    # - token embedding (Adam) uses EMBED_LR
    # - matrix params in transformer blocks use MATRIX_LR via Muon
    # - vectors/scalars use SCALAR_LR via Adam
    block_named_params = list(base_encoder.blocks.named_parameters())
    matrix_params = [
        p
    ...
    optimizer_tok = torch.optim.Adam(
        [
            {
                "params": [base_encoder.tok_emb.weight],
                "lr": token_lr,
                "base_lr": token_lr,
            }
        ],
